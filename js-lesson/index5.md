...
かっこの中の条件の書き方についてですが、いろいろあるので代表的な以下の条件を見ていきましょう。
- 完全に一致
- より大きい
- 以上
- 以下
...
【日本語】
```
- 変数joukenに格納された値が7と完全に一致する場合はコンソールに「ラッキーセブン！」と表示
- 7以外の場合は「残念！！」と表示
```
...
```
<img src="./img/branch03.png" width="50%"/>
...
【日本語】
```
- 変数joukenに格納された値が7よりも大きい場合はコンソールに「ビッグ！」と表示
- それ以外の場合は「スモール・・・」と表示
```
...
```
<img src="./img/branch04.png" width="50%"/>


#### 以上のパターン
【日本語】
```
- 変数joukenに格納された値が7以上の場合はコンソールに「ビッグ！」と表示
- それ以外の場合は「スモール・・・」と表示
```
...
```
<img src="./img/branch05.png" width="50%"/>


#### 以下のパターン
【日本語】
```
- 変数joukenに格納された値が7以下の場合はコンソールに「スモール・・・」と表示
- それ以外の場合は「ビッグ！」と表示
```
...
```
<img src="./img/branch06.png" width="50%"/>


#### 未満のパターン
【日本語】
```
- 変数joukenに格納された値が7未満の場合はコンソールに「スモール・・・」と表示
- それ以外の場合は「ビッグ！」と表示
```
...
```
<img src="./img/branch07.png" width="50%"/>



### 分岐が複数ある場合
条件を付けて分岐するのには慣れてきましたか？

次は複数の分岐がある場合について説明をします。
これも日本語を書いてそのあとにコードを載せておきます。

【日本語】
```
- 変数joukenに格納された値がredの場合はコンソールに「レッドスネーク、щ(ﾟДﾟщ)ｶﾓｰﾝ！」と表示
- 変数joukenに格納された値がyellowの場合はコンソールに「イエロースネーク、щ(ﾟДﾟщ)ｶﾓｰﾝ！」と表示
- それ以外の場合は「ノースネーク・・・」と表示
```
...
```
<img src="./img/branch08.png" width="50%"/>

ポイントは***else if***という書き方ですね。
else ifを続けていくことで2つ以上の分岐を作りことが可能です。
他は今までと同じです。


### 条件文の処理も順次処理
もし1つ目の条件にも2つ目の条件にも当てはまってしまうデータの場合に、処理がどうなるかを見てみましょう。


【日本語】
```
- 変数joukenに格納された値は100
- 値が10以上の場合はコンソール画面に「10以上です」と表示
- 値が20以上の場合はコンソール画面に「20以上です」と表示
- それ以外の場合は「大きい数ですね」と表示
```
...
if(jouken >= 10){
  ...
  console.log("10未満の数字です");
}
```
<img src="./img/branch09.png" width="50%"/>

1つ目の条件にも2つ目の条件にも当てはまっていますが、コンソール画面の結果を見ると1つ目の条件での処理だけが書かれています。

条件文も順次処理で行われるため、書いた順番で条件をチェックします。
そして最初に条件に当てはまった場所で処理を行います。

if文は分岐も含めてif文全体で1つの処理となるため、条件に当てはまった場所で処理を行ったら条件分岐の処理はそこで終了です。

したがってif文で複数の条件を連ねる場合は、条件を書く順番に注意する必要があります。



###まとめ
if文で条件によって処理を変えることができます。

ここでは紹介しませんでしたが、複数の条件に合致した場合にだけ処理を行うand条件や、複数の条件のうちの1つに当てはまれば処理を行うor条件など、条件文の書き方は他にもあります。それぞれ特別な記号を使うので興味があれば調べて、実際にコードを書いてみてください。
